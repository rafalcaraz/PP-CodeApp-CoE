...
## ✅ Done — Admin access gate (preflight permission probe)

Shipped. `<AdminAccessGate>` in `src/components/AdminAccessGate.tsx` wraps
`<AppShell>` inside `<UserLookupProvider>`. At boot it calls
`probeAdminAccess()` (a `take(1)` against the environments table, cache
bypassed) and classifies any failure via `classifyConnectorError()` in
`src/data/inventory.ts`. The classifier covers 403 → `<NoAccessPane>`
(PIM activate link + Re-check), 401 → `<ConnectionBrokenPane>`,
429/503/network → `<TransientErrorPane>` with two auto-retries on
jittered backoff, anything else → `<GenericErrorPane>` with raw
message. Granted result is cached at module scope for the session
lifetime; the Re-check button resets it and re-probes. State is exposed
via `useAdminAccess()` (`src/hooks/useAdminAccess.ts`) for any future
Settings "Access" panel.

Deferred (per the original roadmap notes):

- Session-expiry chokepoint detection (Plan B in the original spec) —
  v1 picked Plan A (let the next query 403, user refreshes, gate
  re-probes). Revisit if users actually hit PIM expiry mid-session
  often enough to complain.
- Limited-mode Settings + Cmd+K when `denied` — gate currently blocks
  everything on failure. Open the design conversation again the first
  time someone wants to flip a feature flag while locked out.
